...
public class FrequencyQueries {
	...
	static List<Integer> freqQuery(List<List<Integer>> queries) {
		...
		for (List<Integer> list : queries) {
			Integer resultFromRule = validateRule(list, arr);
			if(resultFromRule != -1) {
	...
	private static Integer validateRule(List<Integer> list, ArrayList<Integer> arr) {
		Integer result = -1;
		switch (list.get(0)) {
		case 1:
			addOnList(list, arr);
	...
	private static Integer validateList(List<Integer> list, ArrayList<Integer> arr) {
	...
	private static void removeFromList(List<Integer> list, ArrayList<Integer> arr) {
		if(arr.contains(list.get(1))) {
			arr.remove(list.get(1));
		}
	}

	private static void addOnList(List<Integer> list, ArrayList<Integer> arr) {
		arr.add(list.get(1));
	}
}
